...
def detokenize(pred_token_test_path, pred_label_test_path):
    ...
    print(f"pred_token_test_path: {pred_token_test_path} \n pred_label_test_path: {pred_label_test_path}")
    with open(pred_token_test_path,'r',encoding = "ISO-8859-1") as in_tok, open(pred_label_test_path,'r',encoding = "ISO-8859-1") as in_lab: #'token_test.txt'
        ...
        for idx, (bpred_t, bpred_l) in enumerate(zip(bert_pred['toks'], bert_pred['labels'])):
            ...
            if bpred_t=='[SEP]':
                try:
                    ...
                    count_skip += 1
                    print(f"skipped {count_skip} / count: {count}")
                    continue
            elif bpred_t=='[CLS]':
                pass
            else:
